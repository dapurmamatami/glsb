...
function add(){
		...
		if($amount > 0)
		{
				if($wallet_balance >= $amount){
					...
					$id = addWithdrawRequest($request_date, $user_id, $amount, $remark, $bank_id, $bank_name, $bank_account_no, 1);
			...
			if($status_id == 1) { // approve
				...
				if($wallet_balance >= $amount or $type_id == 2) { 
					...
					if($type_id == 1) { //manual withdraw
						...
						wallet('acct_ewallet', 19, $user_id, $trans_description, $amount_in, $manual_withdrawal_charge, $register_user_name, $trans_datetime, $sod_id, $forward_sw,  0, $request_id);
