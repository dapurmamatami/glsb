...
function addSOrder()
{
		...
		foreach($dataB  as $product) {		
				...
				$start_calc_date = $product->start_calc_date;
				
		}
		
		$start_calc_date = ($start_calc_date != "") ? "" . date("Y-m-d", strtotime($start_calc_date)) . "" : NULL;
			...
			if($qty_validation_ok == 1) {
				...
				if($paid_by_ewallet_sw == 0 or ($paid_by_ewallet_sw == 1 and $wallet_balance > $validate_total_amount)) {
					...
					if($courier_sw == 0 or ($courier_sw == 1 and $so_address != '')) {
						...
						$so_id = addSaleOrder($customer_id, $so_customer_name, $so_address, $courier_sw, $courier_amount, $paid_by_ewallet_sw,$upline_id_all, $so_no, $internal_remark, $period_id, $start_calc_date);
...
function changeOrderStatus()
{
	...
	if($id != '')
	{
		...
		{ 
			...
			{
				if(!checkProductNamewithProduct($id)) 
				{
					$file_name = 'App' . $file_name . '.pdf';
				...
				{
					//show error message
					$displayMsg = "Period Name Not Existing in Product";
					$jsonArray= array('id' => 0,'success' => 0, 'displayMsg' => $displayMsg); 
					echo json_encode($jsonArray);
				}
...
function update()
{
		...
		if($courier_sw == 0 or ($courier_sw == 1 and $so_address!=""))
		{
				...
				updateSaleOrderOnly($id, $so_address, $delivery_date, $delivery_courier_company, $delivery_courier_ref_no, $internal_remark, $period_id, $start_calc_date);
...
{
		$id = $_GET['id'];
		$rate_date = ($_POST['rate_date'] != "") ? "" . date("Y-m-d", strtotime($_POST['rate_date'])) . "" : NULL;
		$amount = $_POST[amount];	
		$sod_id = $_POST[sod_id];	

		if($amount!= "")
		{
			addSalesOrderDetailRate($id, $sod_id, $rate_date, $amount);
		...
		{
			//show error message
			$displayMsg = "Missing Amount";
			$jsonArray= array('id' => 0,'success' => 0, 'displayMsg' => $displayMsg); 
			echo json_encode($jsonArray);
		}	
}

function updateSODRate()
{
		$id = $_GET['id'];
		...
		if($amount!= "")
		{
			updateSalesOrderDetailRate($rate_id, $sod_id, $rate_date, $amount);
...
function deleteSODRate()
{
	...
	{
		//delete database
		deleteTableDetail('sorder_detail_rate', 'rate_id', $rate_id);	
