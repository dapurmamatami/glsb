                        ...
                        <div class="tab-content">
                          <div class="active tab-pane" id="detail">
                                ...
                                <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label for="inputName" class="col-sm-2 control-label">Start Calculate Date</label>
                                            <div class="col-sm-2">
                                            <input name="start_calc_date" id="start_calc_date" class="form-control" />
                                            </div>                                                                        
                                     	</div>
...
$(document).ready(function () {  
		...
		$('#rate_date_add').datepicker({ dateFormat: 'dd-mm-yy' });
		$('#rate_date').datepicker({ dateFormat: 'dd-mm-yy' });
		
		var id = '<?php echo $_GET['id']; ?>';
		
		//start of grid control
		var source =
		{
			datatype: "json",
			datafields: [
			...
			cache: false,
			url: 'dataSalesOrderDetailRate.php?id=' + id,
			filter: function()
			{
		...
		$("#jqxgrid").jqxGrid(
			{	
				...
				renderstatusbar: function (statusbar) {
                    ...
                    addButton.click(function (event) {
                        var offset = $("#jqxgrid-Invoice").offset();
                        $("#popupWindowSalesOrderDetailRateAdd").jqxWindow('open');
                        $("#popupWindowSalesOrderDetailRateAdd").jqxWindow('move', offset.left + 30, offset.top + 30);
                    });									
				},			
				<?php } ?>	
				...
				<?php if(checkAccess($_SESSION['user_grp'], basename(dirname($_SERVER['PHP_SELF'])), 'update_sw')){ ?>
		   			{ text: '', datafield: 'Edit', columntype: 'button',pinned: true , width: 40, cellsrenderer: function () {
						 ...
						 $("#rate_date").val(dataRecord.rate_date);
						 $("#amount").val(dataRecord.amount);
						 $("#sod_id").val(dataRecord.sod_id);						 
	
						 // show the popup window.
						 $("#popupWindowSalesOrderDetailRateUpdate").jqxWindow('open');
						 $("#popupWindowSalesOrderDetailRateUpdate").jqxWindow('move', offset.left + 30, offset.top + 30);
					 }
					},		
				<?php } ?>
				...
				<?php if(checkAccess($_SESSION['user_grp'], basename(dirname($_SERVER['PHP_SELF'])), 'delete_sw')){ ?>
 				{ text: '',width: 30, datafield: 'Delete', columntype: 'button',pinned: true , cellsrenderer: function () {
                     ...
                     var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', editrow);
											...
											var form_data = {
												//id: $("#id").val(),
												rate_id: dataRecord.rate_id,
												is_ajax: 1
											};
												
												$.ajax({
													type: "POST",
													url: "info_inc.php?action=deleteSODRate&id="+id,
													data: form_data,
				...
				<?php } ?>						
					...
					{ text: 'Rate Date', editable: false, datafield: 'rate_date', width: 150 },
					{ text: 'Amount (RM)', editable: false, datafield: 'amount', width: 150 },
					//{ text: 'Sales Order ID', editable: false, datafield: 'so_id', width: 150 },
					{ text: 'Sales Order Detail ID', editable: false, datafield: 'product_name', width: 150 },
				]
			});
			
			// create jqxWindow.
            $("#popupWindowSalesOrderDetailRateAdd").jqxWindow({ resizable: false,  autoOpen: false, width: 210, height: 230,cancelButton: $("#Cancel"), });
			$("#popupWindowSalesOrderDetailRateUpdate").jqxWindow({ resizable: false,  autoOpen: false, width: 210, height: 230,cancelButton: $("#Cancel"), });
			//end of grid control
			
			
			//add into database
			$('#buttonSODRateAdd').on('click', function () {
                $('#theSalesOrderDetailRateAdd').jqxValidator('validate');
            });
			
            // initialize validator.
            $('#theSalesOrderDetailRateAdd').jqxValidator({
                rules: [
						 { input: '#rate_date_add', message: 'Required', action: 'keyup, blur', rule: 'required' },
						 { input: '#amount_add', message: 'Required', action: 'keyup, blur', rule: 'required' },
					   ]
            });
			
			//validate success & submit				
			$('#theSalesOrderDetailRateAdd').bind('validationSuccess', function (event) { 
					
			var action = $("#theSalesOrderDetailRateAdd").attr('action');
							
			var form_data = {
				rate_date: $("#rate_date_add").val(),
				amount: $("#amount_add").val(),
				sod_id: $("#sod_id_add option:selected").val(),
				is_ajax: 1
			};
			...
			$('#buttonSODRateUpdate').on('click', function () {
                $('#theSalesOrderDetailRateUpdate').jqxValidator('validate');
            });
			
            // initialize validator.
            $('#theSalesOrderDetailRateUpdate').jqxValidator({
                rules: [
							{ input: '#rate_date', message: 'Required', action: 'keyup, blur', rule: 'required' },
							{ input: '#amount', message: 'Required', action: 'keyup, blur', rule: 'required' },
					   ]
            });
			
			//validate success & submit				
			$('#theSalesOrderDetailRateUpdate').bind('validationSuccess', function (event) { 
					
			var action = $("#theSalesOrderDetailRateUpdate").attr('action');
							
			var form_data = {
				rate_id: $("#rate_id").val(),
				rate_date: $("#rate_date").val(),
				amount: $("#amount").val(),
				sod_id: $("#sod_id option:selected").val(),
				is_ajax: 1
			};
		...
		<div class="box box-info">
            <div class="box-body">                                 
				<div class="col-md-12">
                        ...
                        <li class="active"><a href="#detail" data-toggle="tab">Sales Order Info</a></li>	
                       	<li><a href="#sales_order_detail_rate" data-toggle="tab">Sales Order Detail Info</a></li>
                        </ul>
                            ...
                            <div class="tab-content">
                                ...
                                <div class="box-header with-border">
                                    ...
                                    <input type="submit" value="Print" id="buttonPrint" class="btn btn-primary" onClick="document.theForm.action='info_inc.php?action=printReport&id=<?php if($data[status_id]==5) { echo 'App'.$data[file_name]; } else { echo $data[file_name]; } ?>'"/>                
                                            ...
                                            <div class="form-group">
                                                <label for="inputName" class="col-sm-2 control-label">Start Calculate Date</label>
                                                <div class="col-sm-2">
                                                <input name="start_calc_date" id="start_calc_date" class="form-control" value="<?php echo $data['start_calc_date']; ?>"/>
                                                </div>                                                                        
                                     		</div>
                                            ...
                                            <div class="form-group">
                                                <label for="inputExperience" class="col-sm-2 control-label">Delivery Date</label>
                                                <div class="col-sm-2">
                                            ...
                                            <div class="form-group">     
                                                <label for="inputExperience" class="col-sm-2 control-label">Courier Company</label>
                                                <div class="col-sm-2">
                                                <input type="text" class="form-control" id="delivery_courier_company"  value="<?php echo $data['delivery_courier_company']; ?>" />   
                                                </div>  
                                                <label for="inputExperience" class="col-sm-2 control-label">Courier Ref No</label>
                                                <div class="col-sm-2">
                                            ...
                                            <div class="form-group">
												...
												<?php
                                                $sql = "SELECT sorder_detail.* FROM sorder_detail left join product on sorder_detail.product_id = product.product_id 
												where sorder_detail.so_id = $id order by sod_id";
                                ...
                                <div class="tab-pane" id="sales_order_detail_rate"> 
                                    <div class="post">
                                    <form action="" method="post" name="theSalesOrderDetailRateAdd" id="theSalesOrderDetailRateAdd">
                                    <div id="popupWindowSalesOrderDetailRateAdd">
									<div>
										Add Record</div>
                                        <div style="overflow: hidden;">
                                            ...
                                            <div style='margin-top:5px;'>
                                                <input id='rate_date_add' type="text" class="jqx-input" style="width: 200px; height: 23px;" />
                                            </div> 
                                                                                                           
                                            <div>
                                                Amount (RM):</div>
                                            <div style='margin-top:5px;'>
                                                <input id='amount_add' type="text" class="jqx-input" style="width: 200px; height: 23px;" />
                                            </div> 
                                            
                                            <div>
                                                Amount (RM):</div>
                                            <div style='margin-top:5px;'>
                                                <select name='sod_id_add' id='sod_id_add' class="form-control">
                                                    <?php
                                                        ...
                                                        if(dbNumRows($result)>0)
                                                        {
                                                            while($row=dbFetchAssoc($result))
                                                            {
                                                                ...
                                                                else
                                                                {
                                                                    ...
                                                                    echo "<option value='$row[sod_id]' $cSelect>$row[product_name]</option>"; 
                                                            }
                                                        }
                                                    ?>
                                                    </select>
                                            ...
                                            <div style='margin-top:5px;'>
                                                <div>
                                                    <input style="margin-right: 5px;" type="button" id="buttonSODRateAdd" value="Add" onclick="document.theSalesOrderDetailRateAdd.action='info_inc.php?action=addSODRate&id=<?php echo $_GET[id]; ?>'"/><input id="Cancel" type="button" value="Cancel" />
                                                </div>
                                            </div>
                                  </div>
                            </div>		
                                    ...
                                    <form action="" method="post" name="theSalesOrderDetailRateUpdate" id="theSalesOrderDetailRateUpdate">
                                    <div id="popupWindowSalesOrderDetailRateUpdate">
									<div>
										Add Record</div>
                                        <div style="overflow: hidden;">
                                            <div>
                                                Rate Date:</div>
                                            <div style='margin-top:5px;'>
                                                <input id='rate_date' type="text" class="jqx-input" style="width: 200px; height: 23px;" />
                                            </div> 
                                                                                                           
                                            <div>
                                                Amount (RM):</div>
                                            <div style='margin-top:5px;'>
                                                <input id='amount' type="text" class="jqx-input" style="width: 200px; height: 23px;" />
                                            </div> 
                                            
                                            <div>
                                                Amount (RM):</div>
                                            <div style='margin-top:5px;'>
                                                <select name='sod_id' id='sod_id' class="form-control">
                                                    <?php
                                                        $sql = "SELECT * FROM sorder_detail where so_id = '$id'";
                                            ...
                                            <div style='margin-top:5px;'>
                                                <div>
                                                    <input style="margin-right: 5px;" type="button" id="buttonSODRateUpdate" value="Update" onclick="document.theSalesOrderDetailRateUpdate.action='info_inc.php?action=updateSODRate&id=<?php echo $_GET[id]; ?>'"/><input id="Cancel" type="button" value="Cancel" />
                                                </div>
                                            </div>
                                  </div>
                            </div>		
