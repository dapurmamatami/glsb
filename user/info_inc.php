...
switch ($action) {
  ...
  case 'cancel' :
	...
	case 'resetPasswordWithCharges' :
    resetPasswordWithCharges();
    break;
  }
...
function update()
{
	...
	if($id != '')
	{
		...
		if(!checkUserName($user_name) or $original_user_name <> '') {		
			//update database
			updateUserRegistrationDetail($id, $name, $ic_no, $nationality_id, $nationality_name, $address1, $postcode, $city, $state_id, $state_prefix, $country_id, $country_name, $prefix_name, $tel, $email, $bank_id, $bank_name, $bank_swift_code, $bank_account_no, $user_name, $password, $temp_password, $user_group, $remark, $business_name);
...
function resetPassword()
{
	...
	if($id != '')
	{
		...
		updateTempPassword($u_id, $password, $temp_password);
...
function resetPasswordWithCharges()
{
	$id = $_GET['id'];
	...
	if($id != '')
	{
		...
		$admin_charge = $setupData[admin_charge];
		
		//update database
		insertEmailSend('forgotPassword', $attachment_path, $user_id, 0, $temp_password);
		updateTempPassword($u_id, $password, $temp_password);
		wallet('acct_ewallet', 30, $user_id, 'Reset Password with Charges', 0, $admin_charge, 0, 0, 0, 0, 0, 0);
