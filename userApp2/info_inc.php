...
function newuser()
{
			$error_msg = 'Missing Info';
			echo '<script typ=javascript> alert(" ';
			echo $error_msg;
			echo '"); </script>';	
}
...
function update()
{
	...
	if($id != '')
	{
		...
		//update database
		updateUserRegistrationDetailApp($id, $name, $ic_no, $nationality_id, $nationality_name, $address1, $postcode, $city, $state_id, $state_prefix, $country_id, $country_name, $prefix_name, $tel, $email, $bank_id, $bank_name, $bank_swift_code, $bank_account_no, $user_name, $password, $user_group, $remark);
		
		
		
			header("Location: index.php?view=detail&id=$id");				
	}
	else
	{
			header("Location: index.php?view=detail&id=$id");	
