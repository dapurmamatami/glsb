				...
				if($paid_by_ewallet_sw == 0 or ($paid_by_ewallet_sw == 1 and $wallet_balance > $validate_total_amount)) {
					...
					if($courier_sw == 0 or ($courier_sw == 1 and $so_address != '')) {
						...
						if($paid_by_ewallet_sw == 1)  { //use ewallet to deduct sorder
							...
							$trans_description = "Sales Order ID: " . $so_id;
...
function update()
{
		...
		if($courier_sw == 0 or ($courier_sw == 1 and $so_address!=""))
		{
				...
				updateSaleOrderOnly($id, $so_address, $delivery_date, $delivery_courier_company, $delivery_courier_ref_no);
...
if($_GET['sub'] == 'AllOrder'){
		...
		header("Location: ../../cgi-bin/repwebserver.dll/execute.pdf?reportname=\sorder/$report_name&aliasname=glsb&username=admin&password=&Parammain_id=$main_id&ParamDate_From=$date_from&ParamDate_To=$date_to");		
		}
