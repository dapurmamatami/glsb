<?php
function addWithdrawRequest2($request_date, $user_id, $amount, $remark, $bank_id, $bank_name, $bank_account_no, $type_id) {

			$login_id = (!is_null($_SESSION['user_id'])) ? $_SESSION['user_id'] : 0;
			//add database
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "Insert into acct_ewallet_withdraw_request (request_date, type_id, user_id, 
			amount, remark, bank_id, bank_name, bank_account_no, created_by, created_date) 
			values (NOW(),:type_id, :user_id, :amount, :remark, :bank_id, :bank_name, :bank_account_no, :created_by, NOW())";
            $q = $pdo->prepare($sql);
...
//update to sorder detail rate
function updateSalesOrderDetailRateTransferWithdrawRequest($rate_id, $request_id)
{
		
			//update database
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);							
			$sql = "UPDATE sorder_detail_rate set request_id = :request_id, modified_by = :modified_by WHERE rate_id = :rate_id";			
			$q = $pdo->prepare($sql);
...
function autoTransferWithdrawRequest($dateTransfer) 
{
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $sql = "SELECT *, sorder_detail_rate.amount as sdr_amount from sorder_detail_rate 
				left join sorder on sorder.so_id = sorder_detail_rate.so_id
				left join user on user.user_id = sorder.customer_id
				where request_id = 0 and '$dateTransfer' <= rate_date";
        $q = $pdo->prepare($sql);
        $q->execute();
  		...
  		while ($row = $q->fetch(PDO::FETCH_ASSOC)) 
		{
			...
			$request_id = addWithdrawRequest2($rate_date, $customer_id, $sdr_amount, $internal_remark, $bank_id, $bank_name, $bank_account_no, 2);
			updateSalesOrderDetailRateTransferWithdrawRequest($rate_id, $request_id);
		}			    	
		
		Database::disconnect();
}

function calcLastPeriod($calc_date) 
{
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $sql = "SELECT *, pcr.p_personal_comm as pcr_p_personal_comm from sorder s 
				inner join sorder_detail sd on s.so_id = sd.so_id
				inner join commission_period cp on s.period_id = cp.period_id
				inner join product on sd.product_id = product.product_id
				inner join product_comm_rate pcr on pcr.period_id = s.period_id and pcr.product_id = product.product_id
				where s.status_id = 5 and last_calc_date <= '$calc_date' group by s.so_id";
        $q = $pdo->prepare($sql);
        $q->execute();
        //$data = $q->fetch(PDO::FETCH_ASSOC);
  		...
  		while ($row = $q->fetch(PDO::FETCH_ASSOC)) 
		{
			...
			while ($row2 = $q2->fetch(PDO::FETCH_ASSOC)) 
			{
				...
				$sod_id = $row2[sod_id];
				$last_calc_date = $row2[last_calc_date];
				$period_count = $row2[period_count];
				$period_count_name = $row2[period_count_name];
				$pcr_p_personal_comm = $row2[pcr_p_personal_comm];
				
				$new_calc_last_date = strtotime("+" .$period_count. $period_count_name, strtotime($last_calc_date));
				$new_calc_last_date = date('Y-m-d', $new_calc_last_date);
	

				addSalesOrderDetailRate($so_id, $sod_id, $new_calc_last_date, $pcr_p_personal_comm);
			}
			
			updateLastCalcDate($so_id, $new_calc_last_date);	
		}			    	
...
//update to sorder detail rate
function updateLastCalcDate($so_id, $last_calc_date)
{
		
			//update database
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);							
			$sql = "UPDATE sorder set last_calc_date = :last_calc_date, modified_by = :modified_by WHERE so_id = :so_id";			
			$q = $pdo->prepare($sql);
			$q->bindValue(':so_id', $so_id);
			$q->bindValue(':last_calc_date',!empty($last_calc_date) ? $last_calc_date : NULL);
			$q->bindValue(':modified_by', $_SESSION[user_id]);
			$update = $q->execute();
			Database::disconnect();			
}

function approveSOder2($id) { 
	...
	$start_calc_date = $sorderData[start_calc_date];
	...
	if($status_id == 1) { // pending order
			...
			updateLastCalcDate($id, $start_calc_date);
...
function checkProductNamewithProduct($id) 
{
	if ($id != '')
	{
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM `sorder` s inner join sorder_detail sd on s.so_id = sd.so_id where s.so_id = '$id' and period_id not in (select period_id from product_comm_rate where sd.product_id = product_comm_rate.product_id)";
        $q = $pdo->prepare($sql);
        $q->execute();
        $data = $q->fetch(PDO::FETCH_ASSOC);
        Database::disconnect();
...
//update to sorder detail rate
function updateSalesOrderDetailRate($rate_id, $sod_id, $rate_date, $amount)
{
		
			//update database
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);							
			$sql = "UPDATE sorder_detail_rate set sod_id = :sod_id, rate_date = :rate_date, amount = :amount, modified_by = :modified_by WHERE rate_id = :rate_id";			
			$q = $pdo->prepare($sql);
			$q->bindValue(':rate_id', $rate_id);
			$q->bindValue(':sod_id',!empty($sod_id) ? $sod_id : 0);
			$q->bindValue(':rate_date',!empty($rate_date) ? $rate_date : 0);
			$q->bindValue(':amount',!empty($amount) ? $amount : 0);
			$q->bindValue(':modified_by', $_SESSION[user_id]);
			$update = $q->execute();
			Database::disconnect();			
...
//add to sorder detail rate
function addSalesOrderDetailRate($so_id, $sod_id, $rate_date, $amount)
{
		
			//add database
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "Insert into sorder_detail_rate (so_id, sod_id, rate_date, amount, created_by, created_date) values (:so_id, :sod_id, :rate_date, :amount, :created_by, NOW())";
            $q = $pdo->prepare($sql);
			$q->bindValue(':so_id', $so_id);
			$q->bindValue(':sod_id',!empty($sod_id) ? $sod_id : 0);
			$q->bindValue(':rate_date',!empty($rate_date) ? $rate_date : 0);
...
//add to product
function addProductCommissionRate($product_id, $period_id, $p_comm_level1, $p_comm_level2, $p_comm_level3, $p_personal_comm)
{
		
			//add database
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "Insert into product_comm_rate (product_id, period_id, p_comm_level1, p_comm_level2, p_comm_level3, p_personal_comm, created_by, created_date) 
			values (:product_id, :period_id, :p_comm_level1, :p_comm_level2, :p_comm_level3, :p_personal_comm, :created_by, NOW())";
            $q = $pdo->prepare($sql);
			$q->bindValue(':product_id',!empty($product_id) ? $product_id : 0);
...
//add to closing validate
function addClosing($close_date, $status_id, $monthly_closing_sw, $daily_closing_sw, $remark)
{
			//add database
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "Insert into validate_closing (close_date, status_id, monthly_closing_sw, daily_closing_sw, remark) 
			values (:close_date, :status_id, :monthly_closing_sw, :daily_closing_sw, :remark)";
            $q = $pdo->prepare($sql);
			$q->bindValue(':close_date',!empty($close_date) ? $close_date : NULL);
			$q->bindValue(':status_id',!empty($status_id) ? $status_id : 0);
			$q->bindValue(':monthly_closing_sw',!empty($monthly_closing_sw) ? $monthly_closing_sw : 0);
			$q->bindValue(':daily_closing_sw',!empty($daily_closing_sw) ? $daily_closing_sw : 0);
			$q->bindValue(':remark',!empty($remark) ? $remark : NULL);
			$q->execute();
            $id = $pdo->lastInsertId();
			return $id;
			Database::disconnect();			
...
function checkClosing($close_date, $monthly_closing_sw, $daily_closing_sw)
{

			$check_year = date('Y', strtotime($close_date));	
			$check_month = date('m', strtotime($close_date));	
	
			//add database
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			if ($monthly_closing_sw == 1) 
			{
          		$sql = "SELECT * FROM validate_closing where status_id = 1 and monthly_closing_sw = '$monthly_closing_sw' and year(close_date) = '$check_year' and month(close_date) = '$check_month'";
			}
			if ($daily_closing_sw == 1) 
			{
          		$sql = "SELECT * FROM validate_closing where status_id = 1 and daily_closing_sw = '$daily_closing_sw' and close_date = '$close_date'";
			}
            $q = $pdo->prepare($sql);
			$q->execute();	
			$data = $q->fetch(PDO::FETCH_ASSOC);
...
function TotalQualifyMembersDashboard()
{
	...
	return $total_member_qualify;
}

function TotalNonSalesChargesThisYear()
{	
	$year = date("Y");
	
	$pdo = Database::connect();
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT COALESCE(SUM(amount_out), 0) as total_amount_out_year FROM acct_ewallet WHERE type_id = 30 and year(trans_date) = '$year'";
	$q = $pdo->prepare($sql);
	$q->execute();
	$data = $q->fetch(PDO::FETCH_ASSOC);
	Database::disconnect();

	return $data['total_amount_out_year'];	
}

function TotalNonSalesChargesToday()
{	
	$today = date("Y-m-d");
	
	$pdo = Database::connect();
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT COALESCE(SUM(amount_out), 0) as total_amount_out_today FROM acct_ewallet WHERE type_id = 30 and date(trans_date) = '$today'";
	$q = $pdo->prepare($sql);
	$q->execute();
	$data = $q->fetch(PDO::FETCH_ASSOC);
	Database::disconnect();

	return $data['total_amount_out_today'];	
}

function TotalNonSalesChargesThisMonth()
{
	$this_year = date("Y");
	$this_month = date("m");
	...
	$pdo = Database::connect();
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT COALESCE(SUM(amount_out), 0) as total_amount_out FROM acct_ewallet WHERE type_id = 30 and month(trans_date) = '$this_month' and year(trans_date) = '$this_year'";
	$q = $pdo->prepare($sql);
	$q->execute();
	$data = $q->fetch(PDO::FETCH_ASSOC);
	Database::disconnect();

	return $data['total_amount_out'];	
}

function membershipReminder($month_id) {
	
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $sql = "SELECT u.user_id, u.email, max(so_date) from user u left join sorder s on u.user_id = s.customer_id
				where u.email <> ''
				group by u.user_id
				having 12 * (YEAR(CURDATE())- YEAR(max(so_date))) + (MONTH(CURDATE()) - MONTH(max(so_date))) > '$month_id'";
        $q = $pdo->prepare($sql);
        $q->execute();
		if($month_id == 1)
		{
			$message_type = 'membershipReminder1';
		}
		else if($month_id == 3)
		{
			$message_type = 'membershipReminder2';
		}
		else if($month_id == 5)
		{
			$message_type = 'membershipReminder3';
		}
			while ($row = $q->fetch(PDO::FETCH_ASSOC)) 
			{
				$user_id = $row['user_id'];
				
				insertEmailSend($message_type, 0, $user_id, 0, 0);
			}
		Database::disconnect();
}

function addBeginningBalance($user_id) {
			
	$trans_description = 'Opening Balance';
	$trans_datetime = date("Y-m-d h:i:s");
	
	wallet('acct_ewallet', 0, $user_id, $trans_description, 0, 0, '', $trans_datetime, 0, 0);
	
}

function BonusStatusThisMonthDashboard()
{
	$this_year = date("Y");
	$this_month = date("m");
	...
	$pdo = Database::connect();
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT SUM(amount_in) as total_this_month FROM acct_mwallet WHERE month(trans_date) = '$this_month' and year(trans_date) = '$this_year'";
	$q = $pdo->prepare($sql);
	$q->execute();
	$data = $q->fetch(PDO::FETCH_ASSOC);
	Database::disconnect();

	return $data[total_this_month];		
}

function getPayoutDetailDashboard()
{
		$this_year = date("Y");
        ...
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT 
				COALESCE(SUM(if(month(request_date)='01',amount,0)),0) as total_month1,
				COALESCE(SUM(if(month(request_date)='02',amount,0)),0) as total_month2,
				COALESCE(SUM(if(month(request_date)='03',amount,0)),0) as total_month3, 
				COALESCE(SUM(if(month(request_date)='04',amount,0)),0) as total_month4, 
				COALESCE(SUM(if(month(request_date)='05',amount,0)),0) as total_month5, 
				COALESCE(SUM(if(month(request_date)='06',amount,0)),0) as total_month6, 
				COALESCE(SUM(if(month(request_date)='07',amount,0)),0) as total_month7, 
				COALESCE(SUM(if(month(request_date)='08',amount,0)),0) as total_month8, 
				COALESCE(SUM(if(month(request_date)='09',amount,0)),0) as total_month9, 
				COALESCE(SUM(if(month(request_date)='10',amount,0)),0) as total_month10, 
				COALESCE(SUM(if(month(request_date)='11',amount,0)),0) as total_month11, 
				COALESCE(SUM(if(month(request_date)='12',amount,0)),0) as total_month12
				FROM acct_ewallet_withdraw_request WHERE status_id = 1 and year(request_date) = '$this_year'";
        $q = $pdo->prepare($sql);
        $q->execute();
        $data = $q->fetch(PDO::FETCH_ASSOC);
        Database::disconnect();
...
function getBonusDashoard()
{
        ...
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT 
				COALESCE(SUM(if(month(trans_date)='01',amount_in,0)),0) as total_month1,
				...
				FROM acct_ewallet WHERE year(trans_date) = '$this_year' and type_id in (3)";
        $q = $pdo->prepare($sql);
        $q->execute();
        $data = $q->fetch(PDO::FETCH_ASSOC);
        Database::disconnect();
...
function getSalesMonthDetailDashboard()
{
		$this_year = date("Y");
		$this_month = date("m");
        ...
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT 
				COALESCE(SUM(if(month(approve_date)='01',amount,0)),0) as total_month1,
				COALESCE(SUM(if(month(approve_date)='02',amount,0)),0) as total_month2,
				COALESCE(SUM(if(month(approve_date)='03',amount,0)),0) as total_month3, 
				COALESCE(SUM(if(month(approve_date)='04',amount,0)),0) as total_month4, 
				COALESCE(SUM(if(month(approve_date)='05',amount,0)),0) as total_month5, 
				COALESCE(SUM(if(month(approve_date)='06',amount,0)),0) as total_month6, 
				COALESCE(SUM(if(month(approve_date)='07',amount,0)),0) as total_month7, 
				COALESCE(SUM(if(month(approve_date)='08',amount,0)),0) as total_month8, 
				COALESCE(SUM(if(month(approve_date)='09',amount,0)),0) as total_month9, 
				COALESCE(SUM(if(month(approve_date)='10',amount,0)),0) as total_month10, 
				COALESCE(SUM(if(month(approve_date)='11',amount,0)),0) as total_month11, 
				COALESCE(SUM(if(month(approve_date)='12',amount,0)),0) as total_month12
				FROM sorder WHERE year(approve_date) = '$this_year'";
        $q = $pdo->prepare($sql);
        $q->execute();
        $data = $q->fetch(PDO::FETCH_ASSOC);
        Database::disconnect();
...
function getMemberMonthDetailDashboard($month_id, $year_id)
{

        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT COALESCE(count(user_id), 0) as total_number from user where user_group = 10 and month(approve_date) = '$month_id' and year(approve_date) = '$year_id'";
        $q = $pdo->prepare($sql);
        $q->execute();
        $data = $q->fetch(PDO::FETCH_ASSOC);
        Database::disconnect();
...
function TotalYearSalesDashboard()
{
	$this_year = date("Y");
		...
		$pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT COALESCE(SUM(amount), 0) as total FROM sorder WHERE status_id = 5 and year(approve_date) = '$this_year'";
        $q = $pdo->prepare($sql);
        $q->execute();
        $data = $q->fetch(PDO::FETCH_ASSOC);
        Database::disconnect();
...
function TotalTodaySalesDashboard()
{
	$this_year = date("Y");
	$this_month = date("m");
	$today = date("Y-m-d");
	...
	$pdo = Database::connect();
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT COALESCE(SUM(amount), 0) as total_today FROM sorder WHERE status_id = 5 and date(approve_date) = '$today'";
	$q = $pdo->prepare($sql);
	$q->execute();
	$data = $q->fetch(PDO::FETCH_ASSOC);
	Database::disconnect();

	return $data[total_today];	
}

function TotalThisMonthSalesDashboard()
{
	$this_year = date("Y");
	$this_month = date("m");
	...
	$pdo = Database::connect();
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT COALESCE(SUM(amount), 0) as total_this_month FROM sorder WHERE status_id = 5 and month(approve_date) = '$this_month' and year(approve_date) = '$this_year'";
	$q = $pdo->prepare($sql);
	$q->execute();
	$data = $q->fetch(PDO::FETCH_ASSOC);
	Database::disconnect();
...
function getTotalMemberDashboard($status_id){
	

        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT COALESCE(count(user_id), 0) as total_number from user where user_group = 10 and status_id = '$status_id'";
        $q = $pdo->prepare($sql);
        $q->execute();
        $data = $q->fetch(PDO::FETCH_ASSOC);
        Database::disconnect();
...
function updateTempPassword($u_id, $password, $temp_password)
{
			//update database
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "UPDATE `user`  set `password` = :password, `temp_password` = :temp_password WHERE u_id = :u_id";
            $q = $pdo->prepare($sql);
...
function addAutoWithdrawRequest($year_id, $month_id) {

        
		$companyData = getCompanySetupDetailForm(1);
		$min_balance_to_keep = $companyData['min_balance_to_keep'];
		
		
		$pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT user.*, COALESCE(SUM(amount_in), 0) - COALESCE(SUM(amount_out), 0) as wallet_balance
          FROM acct_ewallet inner join user on acct_ewallet.user_id = user.user_id
          WHERE YEAR(trans_date) = '$year_id' and 
		  Month(trans_date) = '$month_id'
		  group by user.user_id
		  having COALESCE(SUM(amount_in), 0) - COALESCE(SUM(amount_out), 0) > '$min_balance_to_keep'";
        $q = $pdo->prepare($sql);
        $q->execute();
        //$data = $q->fetch(PDO::FETCH_ASSOC);
  		...
  		while ($row = $q->fetch(PDO::FETCH_ASSOC)) {
			...
			$sql2 = "SELECT COALESCE(SUM(amount), 0) as total_amount
					  FROM acct_ewallet_withdraw_request
					  WHERE user_id = '$user_id' and status_id = 0 and (type_id = 1 or type_id=3)";
			$q2 = $pdo->prepare($sql2);
			$q2->execute();
			...
			if($available_balance > 0) {
				...
				addWithdrawRequest($request_date, $user_id, $available_balance, $remark, $bank_id, $bank_name, $bank_account_no, 3);
...
function monthEndClosingWallet($wallet_type, $year_id, $month_id) {
        ...
        $sql = "SELECT user.user_id from user where user_group = 10";
        $q = $pdo->prepare($sql);
        ...
        //$data = $q->fetch(PDO::FETCH_ASSOC);
  		...
  		while ($row = $q->fetch(PDO::FETCH_ASSOC)) {
			...
			$sql2 = "SELECT user_id, COALESCE(SUM(amount_in), 0)  as total_amount_in, 
			COALESCE(SUM(amount_out), 0)  as total_amount_out, 
			COALESCE(SUM(amount_in), 0) - COALESCE(SUM(amount_out), 0) as balance from acct_ewallet
			where year(trans_date) = ? and month(trans_date) = ? and user_id = ?";
			$q2 = $pdo->prepare($sql2);
			$q2->execute(array($year_id, $month_id, $user_id));			
			...
			$firstDay = $year_id . '-' . $month_id . '-01';
			$trans_month = date("Y-m-d", strtotime($firstDay));
			
			$trans_datetime = strtotime('+1 month', strtotime($trans_month));
			$trans_datetime = date('Y-m-d', $trans_datetime);
...
function insertEmailSendPayout($email_subject, $email_message, $email_message_footer, $attachment_path, $user_id, $mass_email_sw, $user_temp_password) {
			...
			$q->bindValue(':email_message_footer', $email_message_footer);
...
function wallet($wallet_type, $type_id, $user_id, $trans_description, $amount_in, $amount_out, $register_user_name, $trans_datetime, $sod_id, $forward_sw, $so_id, $request_id) {
			...
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "Insert into $wallet_type (trans_date, type_id, user_id, trans_description, amount_in, amount_out, so_id, sod_id, forward_sw, request_id, created_by, created_date) 
			values (:trans_datetime, :type_id, :user_id, :trans_description, :amount_in, :amount_out, :so_id, :sod_id, :forward_sw, :request_id, :created_by, NOW())";
            $q = $pdo->prepare($sql);
			$q->bindValue(':type_id', $type_id);
			$q->bindValue(':sod_id', $sod_id);
			$q->bindValue(':so_id',!empty($so_id) ? $so_id : 0);
			$q->bindValue(':trans_datetime', $trans_datetime);
			$q->bindValue(':user_id', $user_id);
	...
	if ($id != ''){
		...
		$trans_datetime = date("Y-m-d h:i:s");
  		...
  		while ($row = $q->fetch(PDO::FETCH_ASSOC)) {
			...
			$personal_comm = $row[personal_comm];
			$personal_comm = $row[personal_comm] * $quantity;
			
			$product_bonus_pool = $product_bonus_pool * $quantity;

			$trans_description = 'Pool Bonus';
			
			wallet('acct_mwallet', 3, $upline_id, $trans_description, $product_bonus_pool, 0, '', $trans_datetime, $sod_id, 0, $id);

			//calc personal commission
			if($personal_comm_sw == 1 and $personal_comm > 0) {
				$trans_description = 'Personal Earning';
				//$trans_datetime = date("Y-m-d h:i:s");				
				wallet('acct_ewallet', 5, $user_id, $trans_description, $personal_comm, 0, '', $trans_datetime, $sod_id, 0, $id);				
			}
									
			//if($upline_id > 0){
			// modify to excluce admin (HQ)
			if($upline_id > 1){
				...
				for ($x = 1; $x <= $total_level; $x++) {
					...
					if($upline_id > 1){
						...
						if($amount_in > 0) {
							...
							//insert into ewallet
							$trans_description = 'Sales Commission Level ' . $x . ' (' . $member_reg_no . ')';
...
//update announcement
function updateAnnouncementDetail($anno_id, $anno_title, $anno_date, $anno_description, $active_sw, $sorting_number){
	
		
			//delete database
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "UPDATE `announcement`  set `anno_title` = :anno_title, anno_date = :anno_date, `anno_description` = :anno_description, `active_sw` = :active_sw, sorting_number = :sorting_number, `modified_by` = :modified_by WHERE anno_id = :anno_id";
            $q = $pdo->prepare($sql);
			...
			$q->bindValue(':sorting_number', $sorting_number);
			$q->bindValue(':modified_by', $_SESSION[user_id]);
			$update = $q->execute();
...
function addAnnouncementDetail($anno_title, $anno_date, $anno_description, $active_sw, $sorting_number)
{
			...
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "Insert into announcement (anno_title, anno_date, anno_description, active_sw, sorting_number, created_by, created_date) values (:anno_title, :anno_date, :anno_description, :active_sw, :sorting_number, :created_by, NOW())";
            $q = $pdo->prepare($sql);
			$q->bindValue(':anno_title', $anno_title);
			$q->bindValue(':anno_date', $anno_date);
			$q->bindValue(':anno_description', $anno_description);
...
function updateUserRegistrationDetail($u_id, $name, $ic_no, $nationality_id, $nationality_name, $address1, $postcode, $city, $state_id, $state_prefix, $country_id, $country_name, $prefix_name, $tel, $email, $bank_id, $bank_name, $bank_swift_code, $bank_account_no, $user_name, $password, $temp_password, $user_group, $remark, $business_name){
			...
			$q->bindValue(':user_group', $user_group);
			$q->bindValue(':remark', $remark);
			$q->bindValue(':business_name', $business_name);
			$q->bindValue(':modified_by', $_SESSION[user_id]);
			$update = $q->execute();
...
function addUserRegistration($name, $ic_no, $nationality_id, $nationality_name, $address1, $postcode, $city, $state_id, $state_prefix, $country_id, $country_name, $prefix_name, $tel, $email, $bank_id, $bank_name, $bank_swift_code, $bank_account_no, $user_name, $password, $temp_password, $user_group, $remark, $member_reg_no,$sponsor_member_reg_no, $upline_id, $business_name)
{
			...
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "Insert into user (name, ic_no, nationality_id, nationality_name, address1, postcode, city, state_id, state_prefix, country_id, country_name, prefix_name, tel, email, bank_id, bank_name, bank_swift_code, bank_account_no, user_name, password, temp_password, user_group, remark, member_reg_no, sponsor_member_reg_no, upline_id, business_name, created_by, created_date) values (:name, :ic_no, :nationality_id, :nationality_name, :address1, :postcode, :city, :state_id, :state_prefix, :country_id, :country_name, :prefix_name, :tel, :email, :bank_id, :bank_name, :bank_swift_code, :bank_account_no, :user_name, :password, :temp_password, :user_group, :remark, :member_reg_no,:sponsor_member_reg_no, :upline_id, :business_name, :created_by, NOW())";
			...
			$q->bindValue(':business_name',!empty($business_name) ? $business_name : NULL);
			$q->bindValue(':created_by',!empty($_SESSION[user_id]) ? $_SESSION[user_id] : 0);
			//$q->bindValue(':created_by', $_SESSION[user_id]);
			$q->execute();
            $id = $pdo->lastInsertId();
...
function updateSaleOrderOnly($so_id, $so_address, $delivery_date, $delivery_courier_company, $delivery_courier_ref_no, $internal_remark, $period_id, $start_calc_date){
	
		
			//update database
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "UPDATE `sorder`  set `so_address` = :so_address, delivery_date = :delivery_date, delivery_courier_company = :delivery_courier_company, delivery_courier_ref_no = :delivery_courier_ref_no, internal_remark = :internal_remark, period_id = :period_id, start_calc_date = :start_calc_date, modified_by = :modified_by WHERE so_id = :so_id";
            $q = $pdo->prepare($sql);
			$q->bindValue(':so_id', $so_id);
			...
			$q->bindValue(':delivery_courier_company', $delivery_courier_company);
			$q->bindValue(':delivery_courier_ref_no', $delivery_courier_ref_no);
			$q->bindValue(':internal_remark', $internal_remark);
			$q->bindValue(':period_id',!empty($period_id) ? $period_id : 0);
			$q->bindValue(':start_calc_date',!empty($start_calc_date) ? $start_calc_date : NULL);
...
//add to sale order
function addSaleOrder($customer_id, $so_customer_name, $so_address, $courier_sw, $courier_amount, $paid_by_ewallet_sw, $upline_id_all, $so_no, $internal_remark, $period_id, $start_calc_date)
{
			...
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "Insert into sorder (so_date,so_no, customer_id, so_customer_name, so_address,courier_sw,courier_amount,paid_by_ewallet_sw ,upline_id_all, internal_remark, period_id, start_calc_date, created_by, created_date) values (NOW(), :so_no, :customer_id, :so_customer_name, :so_address, :courier_sw, :courier_amount,:paid_by_ewallet_sw,:upline_id_all, :internal_remark, :period_id, :start_calc_date, :created_by, NOW())";
            $q = $pdo->prepare($sql);
			...
			$q->bindValue(':internal_remark',!empty($internal_remark) ? $internal_remark : NULL);
			$q->bindValue(':period_id',!empty($period_id) ? $period_id : 0);
			$q->bindValue(':start_calc_date',!empty($start_calc_date) ? $start_calc_date : NULL);
...
//add to sale order
function addSaleOrderDetail($id, $product_id, $product_code, $product_name, $unit_price, $tax_percentage, $unit_price_with_gst, $quantity, $amount, $product_weight, $total_weight, $product_pv, $order_pv, $product_bonus_pool, $product_cost, $p_comm_level1, $p_comm_level2, $p_comm_level3, $p_personal_comm)
{
			...
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "Insert into sorder_detail (so_id, product_id, product_code, product_name, unit_price, tax_percentage, unit_price_with_gst, quantity, amount,product_weight,total_weight,product_pv,order_pv, product_bonus_pool, product_cost, p_comm_level1, p_comm_level2, p_comm_level3, p_personal_comm, created_by, created_date) values (:so_id, :product_id, :product_code, :product_name, :unit_price, :tax_percentage, :unit_price_with_gst, :quantity, :amount,:product_weight,:total_weight,:product_pv,:order_pv,:product_bonus_pool,:product_cost, :p_comm_level1, :p_comm_level2, :p_comm_level3, :p_personal_comm, :created_by, NOW())";
            $q = $pdo->prepare($sql);
			$q->bindValue(':so_id', $id);
			$q->bindValue(':product_id', $product_id);
...
function getUserDetail($id){
	...
	if ($id != ''){
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM user left join state on state.state_id = user.state_id where user_id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
        $data = $q->fetch(PDO::FETCH_ASSOC);
        Database::disconnect();
...
//add to product
function insertProduct($product_code, $product_category, $product_name, $product_short_name, $selling_price, $cost_of_good_sold, $profit, $bonus_pool, $comm_level1, $comm_level2,$comm_level3, $point_value, $gst_rate_type, $gst_rate, $weight_in_gram, $product_description, $personal_comm)
{
		
			//add database
			$pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "Insert into product (product_code, product_category, product_name, product_short_name, selling_price, cost_of_good_sold, profit, bonus_pool, comm_level1, comm_level2, comm_level3, point_value, gst_rate_type, gst_rate,weight_in_gram, product_description,personal_comm, created_by, created_date) 
			values (:product_code, :product_category, :product_name, :product_short_name, :selling_price, :cost_of_good_sold, :profit, :bonus_pool, :comm_level1, :comm_level2, :comm_level3, :point_value, :gst_rate_type, :gst_rate, :weight_in_gram, :product_description,:personal_comm, :created_by, NOW())";
            $q = $pdo->prepare($sql);
			$q->bindValue(':product_code', $product_code);
			...
			$q->bindValue(':personal_comm',!empty($personal_comm) ? $personal_comm : 0);
			$q->bindValue(':created_by',!empty($_SESSION[user_id]) ? $_SESSION[user_id] : 0);
			$q->execute();
            $id = $pdo->lastInsertId();
			return $id;
...
//update Product
function updateProductDetail($product_id, $product_code, $product_category, $product_name, $product_short_name, $selling_price, $cost_of_good_sold, $profit, $bonus_pool, $comm_level1, $comm_level2,$comm_level3, $point_value, $gst_rate_type, $gst_rate , $weight_in_gram, $product_description, $personal_comm){
			...
			`product_name` = :product_name, `product_short_name` = :product_short_name, `selling_price` = :selling_price,
			 `cost_of_good_sold` = :cost_of_good_sold, `profit` = :profit, `bonus_pool` = :bonus_pool, 
			 `comm_level1` = :comm_level1, `comm_level2` = :comm_level2, `comm_level3` = :comm_level3, 
			 `point_value` = :point_value, `gst_rate_type` = :gst_rate_type, `gst_rate` = :gst_rate, 
			 weight_in_gram = :weight_in_gram, product_description = :product_description, 
